...
from factor_lab.hermes_profile_settings import HermesProfileSetting, load_hermes_profile_settings, select_hermes_profile_setting
from factor_lab.data_steward_decision_engine import build_data_steward_response
from factor_lab.diagnostician_engine import build_failure_response
...
from factor_lab.hermes_contract_validation import validate_decision_payload
...
DECISION_SCHEMA_VERSIONS: dict[str, str] = {
    "planner": "factor_lab.researcher_profile_response.v1",
    "diagnostician": "factor_lab.diagnostician_response.v1",
    "reviewer": "factor_lab.reviewer_agent_response.v1",
    "data_steward": "factor_lab.data_steward_agent_response.v1",
}
LEGACY_PROVIDER_ALIASES = {
    "hermes_native_gateway": "legacy_hermes_native_gateway",
    "hermes_native_session": "legacy_hermes_native_gateway",
    "hermes_native_http": "legacy_hermes_native_gateway",
    "hermes_native_agent": "legacy_hermes_native_agent",
    "hermes_native_cli": "legacy_hermes_native_agent",
    "hermes_native_internal": "legacy_hermes_native_agent",
    "hermes_native": "legacy_hermes_native_agent",
}
NORMALIZED_REMOTE_PROVIDER_SOURCES = {
    "direct_model",
    "legacy_hermes_native_gateway",
    "legacy_hermes_native_agent",
}
REMOTE_PROVIDER_SOURCES = {
    "direct_model",
    "hermes_native_gateway",
    "hermes_native_agent",
    "hermes_native_cli",
    "hermes_native_internal",
    *NORMALIZED_REMOTE_PROVIDER_SOURCES,
}
...
class HermesDecisionRouter:
    def __init__(self, provider: str | None = None, model: str | None = None) -> None:
        ...
        self.model = (
            ...
            or os.environ.get("FACTOR_LAB_HERMES_NATIVE_AGENT_TARGET")
    ...
    def _normalized_provider_name(self, provider: str | None = None) -> str:
        ...
        if raw in {"real", "direct_model", "openai", "openai_compatible"}:
            return "direct_model"
    ...
    def generate(self, decision_type: str, context: dict[str, Any]) -> dict[str, Any]:
        builders: dict[str, DecisionBuilder] = {
            ...
            "diagnostician": lambda payload: build_failure_response(payload, source_label="heuristic"),
            "reviewer": lambda payload: build_reviewer_response(payload, source_label="heuristic"),
            "data_steward": lambda payload: build_data_steward_response(payload, source_label="heuristic"),
        }
        mock_builders: dict[str, DecisionBuilder] = {
            "planner": lambda payload: build_planner_response(payload, source_label="mock"),
            "diagnostician": lambda payload: build_failure_response(payload, source_label="mock"),
            "reviewer": lambda payload: build_reviewer_response(payload, source_label="mock"),
            "data_steward": lambda payload: build_data_steward_response(payload, source_label="mock"),
        }
        if decision_type not in builders:
            raise ValueError(f"unsupported decision_type: {decision_type}")

        chain = self._provider_chain()
        hermes_profile = select_hermes_profile_setting(decision_type)
        if hermes_profile is None and self._decision_type_is_configured_but_disabled(decision_type):
            raise RuntimeError(f"Hermes profile for decision_type {decision_type} is disabled")
        ...
        for provider in chain:
            ...
            if provider in {"hermes_native_gateway", "hermes_native_agent"}:
                request_scope_id = self._hermes_native_request_scope_id(context=context)
                session_mode = self._hermes_native_session_mode()
                session_id = self._hermes_native_session_id(
                    decision_type,
                    context=context,
            ...
            try:
                if provider == "direct_model":
                    payload = self._call_direct_model(decision_type, context, hermes_profile=hermes_profile)
                elif provider == "hermes_native_gateway":
                    payload = self._call_hermes_native_gateway(
                        decision_type,
                        context,
                        session_id=session_id,
                        request_scope_id=request_scope_id,
                    )
                elif provider in {"hermes_native_agent", "hermes_native_cli", "hermes_native_internal"}:
                    payload = self._call_hermes_native_agent(
                        decision_type,
                        context,
                        session_id=session_id,
                        request_scope_id=request_scope_id,
                ...
                self._attach_hermes_profile_metadata(payload, decision_type, hermes_profile)
                ...
                if schema_valid or (hermes_profile and not hermes_profile.strict_schema):
                    return payload
    ...
    def _attach_hermes_profile_metadata(self, payload: dict[str, Any], decision_type: str, hermes_profile: HermesProfileSetting | None) -> None:
        metadata = payload.setdefault("decision_metadata", {})
        metadata["hermes_profile"] = hermes_profile.name if hermes_profile else decision_type
        metadata["hermes_profile_source"] = "configured" if hermes_profile else "implicit"
        metadata["hermes_profile_enabled"] = bool(hermes_profile and hermes_profile.enabled)
        metadata["legacy_agent_id"] = hermes_profile.legacy_agent_id if hermes_profile else self._hermes_native_agent_id(decision_type)
        if hermes_profile:
            metadata["hermes_profile_display_name"] = hermes_profile.display_name
            metadata["hermes_profile_decision_types"] = hermes_profile.decision_types
            metadata["hermes_profile_strict_schema"] = hermes_profile.strict_schema
            metadata["hermes_profile_timeout_seconds"] = hermes_profile.timeout_seconds
            metadata["hermes_profile_max_retries"] = hermes_profile.max_retries

    def _metadata_model(self, provider: str, payload: dict[str, Any]) -> str:
        if provider == "hermes_native_gateway":
            meta = payload.get("hermes_native_gateway_meta") or {}
    ...
    def _provider_chain(self) -> list[str]:
        ...
        if normalized_provider == "auto":
            if self._real_provider_configured():
                return ["direct_model", "heuristic", "mock"]
            if self._hermes_native_gateway_configured():
                return ["hermes_native_gateway", "heuristic", "mock"]
            if self._hermes_native_agent_configured():
                return ["hermes_native_agent", "heuristic", "mock"]
            return ["heuristic", "mock"]
        if normalized_provider == "legacy_hermes_native_gateway":
            return ["hermes_native_gateway", "heuristic", "mock"]
        if normalized_provider == "legacy_hermes_native_agent":
            return ["hermes_native_agent", "heuristic", "mock"]
        if normalized_provider == "direct_model":
            return ["direct_model", "heuristic", "mock"]
    ...
    def _real_provider_configured(self) -> bool:
        return bool(self._direct_model_profiles())

    def _hermes_native_agent_configured(self) -> bool:
        explicit_provider = self.provider in {"hermes_native_agent", "hermes_native_cli", "hermes_native_internal", "hermes_native"}
        planner_env = os.environ.get("FACTOR_LAB_HERMES_NATIVE_RESEARCHER_PROFILE")
        failure_env = os.environ.get("FACTOR_LAB_HERMES_NATIVE_FAILURE_AGENT")
        if not explicit_provider and not (planner_env and failure_env):
            return False
        planner_dir = Path.home() / ".hermes_native" / "agents" / self._hermes_native_agent_id("planner") / "agent"
        failure_dir = Path.home() / ".hermes_native" / "agents" / self._hermes_native_agent_id("diagnostician") / "agent"
        return planner_dir.exists() and failure_dir.exists()

    def _hermes_native_gateway_configured(self) -> bool:
        explicit_provider = self.provider in {"auto", "hermes_native_gateway", "hermes_native_session", "hermes_native_http"}
        if not explicit_provider and not os.environ.get("FACTOR_LAB_HERMES_NATIVE_GATEWAY_URL"):
            return False
        return self._hermes_native_agent_configured()
    ...
    def healthcheck(self, output_path: str | Path | None = None, *, probe: bool = True) -> dict[str, Any]:
        ...
        payload: dict[str, Any] = {
            ...
            "hermes_native_gateway_configured": self._hermes_native_gateway_configured(),
            "hermes_native_agent_configured": self._hermes_native_agent_configured(),
            "gateway_url": self._hermes_native_gateway_url(),
            "env": {
                ...
                "FACTOR_LAB_HERMES_NATIVE_RESEARCHER_PROFILE": bool(os.environ.get("FACTOR_LAB_HERMES_NATIVE_RESEARCHER_PROFILE")),
                "FACTOR_LAB_HERMES_NATIVE_FAILURE_AGENT": bool(os.environ.get("FACTOR_LAB_HERMES_NATIVE_FAILURE_AGENT")),
                "FACTOR_LAB_HERMES_NATIVE_SESSION_PREFIX": bool(os.environ.get("FACTOR_LAB_HERMES_NATIVE_SESSION_PREFIX")),
                "FACTOR_LAB_HERMES_NATIVE_BACKEND_MODEL": bool(os.environ.get("FACTOR_LAB_HERMES_NATIVE_BACKEND_MODEL")),
                "FACTOR_LAB_HERMES_NATIVE_GATEWAY_URL": bool(os.environ.get("FACTOR_LAB_HERMES_NATIVE_GATEWAY_URL")),
                "FACTOR_LAB_HERMES_NATIVE_GATEWAY_TOKEN": bool(os.environ.get("FACTOR_LAB_HERMES_NATIVE_GATEWAY_TOKEN") or os.environ.get("HERMES_NATIVE_GATEWAY_TOKEN")),
            },
            ...
            "degraded_to_heuristic": False,
            "researcher_profile": self._hermes_native_agent_id("planner"),
            "failure_agent": self._hermes_native_agent_id("diagnostician"),
            "hermes_profile_settings": [
                {
                    "name": role.name,
        ...
        if self.provider in {"auto", "hermes_native_gateway", "hermes_native_session", "hermes_native_http"} and payload["hermes_native_gateway_configured"]:
            payload["recommended_effective_source"] = "legacy_hermes_native_gateway"
            if probe:
                payload["probe"] = self._probe_hermes_native_gateway(timeout_seconds=timeout)
            else:
        ...
        elif self.provider in {"hermes_native_agent", "hermes_native_cli", "hermes_native_internal", "hermes_native"}:
            payload["recommended_effective_source"] = "legacy_hermes_native_agent" if payload["hermes_native_agent_configured"] else "heuristic"
        elif self.provider in {"real", "direct_model", "openai", "openai_compatible"} and payload["real_provider_configured"]:
            payload["recommended_effective_source"] = "direct_model"
    ...
    def _probe_hermes_native_gateway(self, timeout_seconds: float) -> dict[str, Any]:
        started = time.perf_counter()
        probe = {
            "attempted": True,
            "ok": False,
        ...
        req = urllib.request.Request(
            url=self._hermes_native_gateway_health_url(),
            method="GET",
        )
        try:
    ...
    def _decision_type_is_configured_but_disabled(self, decision_type: str) -> bool:
        ...
        return any(normalized in role.decision_types and not role.enabled for role in load_hermes_profile_settings())

    def _hermes_native_agent_id(self, decision_type: str) -> str:
        role = select_hermes_profile_setting(decision_type)
        if role and role.legacy_agent_id:
            return role.legacy_agent_id
        if decision_type == "planner":
            return os.environ.get("FACTOR_LAB_HERMES_NATIVE_RESEARCHER_PROFILE") or "factor-lab-planner"
        if decision_type == "diagnostician":
            return os.environ.get("FACTOR_LAB_HERMES_NATIVE_FAILURE_AGENT") or "factor-lab-failure"
        env_key = f"FACTOR_LAB_HERMES_NATIVE_{decision_type.upper()}_AGENT"
        return os.environ.get(env_key) or f"factor-lab-{decision_type.replace('_', '-')}"

    def _hermes_native_session_base(self, decision_type: str) -> str:
        prefix = os.environ.get("FACTOR_LAB_HERMES_NATIVE_SESSION_PREFIX") or "factor-lab-decision"
        suffix = decision_type.replace("_", "-") or "decision"
        return f"{prefix}-{suffix}"

    def _hermes_native_session_mode(self, purpose: str = "decision") -> str:
        if purpose == "healthcheck":
            return "ephemeral"
        raw = (os.environ.get("FACTOR_LAB_HERMES_NATIVE_SESSION_MODE") or "ephemeral").strip().lower()
        if raw not in {"ephemeral", "persistent"}:
            return "ephemeral"
        return raw

    def _hermes_native_request_scope_id(self, context: dict[str, Any] | None = None, purpose: str = "decision") -> str:
        if purpose == "healthcheck":
            return uuid4().hex[:12]
    ...
    def _hermes_native_session_id(
        ...
        base = self._hermes_native_session_base(decision_type)
        if self._hermes_native_session_mode(purpose=purpose) == "persistent":
            return base
        scope_id = request_scope_id or self._hermes_native_request_scope_id(context=context, purpose=purpose)
    ...
    def _compact_data_steward_inputs(self, inputs: dict[str, Any]) -> dict[str, Any]:
    ...
    def _compact_context_for_prompt(self, decision_type: str, context: dict[str, Any]) -> dict[str, Any]:
        ...
        elif decision_type == "data_steward":
            compact_inputs = self._compact_data_steward_inputs(inputs)
        else:
    ...
    def _hermes_native_prompt(self, decision_type: str, context: dict[str, Any]) -> str:
        compact_context = self._compact_context_for_prompt(decision_type, context)
        return (
            "You are the Factor Lab decision layer running as an internal HermesNative decision session.\n"
    ...
    def _call_hermes_native_agent(
        ...
        *,
        ...
        agent_id = self._hermes_native_agent_id(decision_type)
        session_id = session_id or self._hermes_native_session_id(
            decision_type,
            context=context,
            request_scope_id=request_scope_id,
        )
        timeout = int(float(os.environ.get("FACTOR_LAB_LLM_TIMEOUT_SECONDS") or 20))
        thinking = os.environ.get("FACTOR_LAB_HERMES_NATIVE_THINKING") or "low"
        prompt = self._hermes_native_prompt(decision_type, context)
        command = [
            "hermes_native",
            "agent",
            "--local",
        ...
        if completed.returncode != 0:
            raise RuntimeError((completed.stderr or completed.stdout or "hermes_native agent failed").strip())
        ...
        payload.setdefault("agent_name", f"{decision_type}-hermes_native-agent")
        payload.setdefault("decision_source", "hermes_native_agent")
        payload.setdefault("decision_context_id", context.get("context_id"))
        payload.setdefault(
            "hermes_native_agent_meta",
            {
                "agent_id": agent_id,
                "session_id": session_id,
    ...
    def _hermes_native_gateway_url(self) -> str:
        raw = (
            os.environ.get("FACTOR_LAB_HERMES_NATIVE_GATEWAY_URL")
            or os.environ.get("HERMES_NATIVE_GATEWAY_URL")
    ...
    def _hermes_native_gateway_health_url(self) -> str:
        raw = (
            os.environ.get("FACTOR_LAB_HERMES_NATIVE_GATEWAY_URL")
            or os.environ.get("HERMES_NATIVE_GATEWAY_URL")
            or "http://127.0.0.1:18789/v1/chat/completions"
    ...
    def _hermes_native_request_model(self, decision_type: str) -> str:
        return f"hermes_native/{self._hermes_native_agent_id(decision_type)}"

    def _hermes_native_gateway_headers(self, session_id: str) -> dict[str, str]:
        headers = {
            "Content-Type": "application/json",
            "x-hermes_native-session-key": session_id,
            "x-hermes_native-message-channel": os.environ.get("FACTOR_LAB_HERMES_NATIVE_MESSAGE_CHANNEL") or "webchat",
        }
        token = os.environ.get("FACTOR_LAB_HERMES_NATIVE_GATEWAY_TOKEN") or os.environ.get("HERMES_NATIVE_GATEWAY_TOKEN")
        if token:
            headers["Authorization"] = f"Bearer {token}"
        backend_model = os.environ.get("FACTOR_LAB_HERMES_NATIVE_BACKEND_MODEL")
        if backend_model:
            headers["x-hermes_native-model"] = backend_model
        return headers

    def _call_hermes_native_gateway(
        self,
        decision_type: str,
        context: dict[str, Any],
        *,
        ...
        request_model = self._hermes_native_request_model(decision_type)
        prompt = self._hermes_native_prompt(decision_type, context)
        request_body = {
            "model": request_model,
            "messages": [
        ...
        req = urllib.request.Request(
            url=self._hermes_native_gateway_url(),
            data=json.dumps(request_body).encode("utf-8"),
            headers=self._hermes_native_gateway_headers(session_id),
            method="POST",
        )
        timeout = float(os.environ.get("FACTOR_LAB_LLM_TIMEOUT_SECONDS") or 20)
        try:
        ...
        payload.setdefault("schema_version", self._decision_schema_version(decision_type))
        payload.setdefault("agent_name", f"{decision_type}-hermes_native-gateway")
        payload.setdefault("decision_source", "hermes_native_gateway")
        payload.setdefault("decision_context_id", context.get("context_id"))
        payload.setdefault(
            "hermes_native_gateway_meta",
            {
                "agent_id": self._hermes_native_agent_id(decision_type),
                "session_id": session_id,
                "request_scope_id": request_scope_id or self._context_request_scope_id(context),
    ...
    def _direct_model_profiles(self, fallback_order: list[str] | None = None) -> list[dict[str, Any]]:
    ...
    def _agent_system_prompt(self, decision_type: str, hermes_profile: HermesProfileSetting | None = None) -> str:
        if hermes_profile and hermes_profile.system_prompt.strip():
            return hermes_profile.system_prompt.strip()
        if decision_type == "planner":
            return "你是 Factor Lab 的规划 agent。必须基于输入 snapshot，不得编造指标。输出必须符合 planner schema。"
        if decision_type == "diagnostician":
            return "你是 Factor Lab 的失败诊断 agent。优先定位根因：数据、配置、模型、schema、回测、候选质量。输出必须符合 diagnostician schema。"
        return (
    ...
    def _direct_model_headers(self, api_key: str, *, auth_scheme: str = "bearer") -> dict[str, str]:
        headers = {
            "Content-Type": "application/json",
    ...
    def _record_direct_model_http_error(
        self,
        *,
    ...
    def _direct_model_endpoint_url(self, base_url: str, api_format: str) -> str:
        raw = base_url.rstrip("/")
    ...
    def _call_direct_model_profile(
        self,
        decision_type: str,
        context: dict[str, Any],
        profile: dict[str, Any],
        hermes_profile: HermesProfileSetting | None = None,
    ) -> dict[str, Any]:
        ...
        system_prompt = self._agent_system_prompt(decision_type, hermes_profile)
        ...
        started = time.perf_counter()
        timeout = float(hermes_profile.timeout_seconds if hermes_profile else (os.environ.get("FACTOR_LAB_LLM_TIMEOUT_SECONDS") or 20))
        if api_format == "anthropic":
            request_body = {
                "model": model,
            ...
            req = urllib.request.Request(
                url=self._direct_model_endpoint_url(base_url, api_format),
                data=json.dumps(request_body).encode("utf-8"),
                headers=self._direct_model_headers(api_key, auth_scheme="anthropic"),
                method="POST",
            )
            try:
                ...
                body = exc.read().decode("utf-8", errors="ignore")
                self._record_direct_model_http_error(
                    exc=exc,
                    body=body,
                    decision_type=decision_type,
                    profile=profile,
            ...
            req = urllib.request.Request(
                url=self._direct_model_endpoint_url(base_url, api_format),
                data=json.dumps(request_body).encode("utf-8"),
                headers=self._direct_model_headers(api_key),
                method="POST",
            )
            try:
                with urllib.request.urlopen(req, timeout=timeout) as response:
        ...
        payload.setdefault("direct_model_cost", cost)
        self._try_append_llm_usage_ledger(
            self._build_llm_usage_ledger_row(
                success=True,
                decision_type=decision_type,
    ...
    def _call_direct_model(
        self,
        decision_type: str,
        context: dict[str, Any],
        hermes_profile: HermesProfileSetting | None = None,
    ) -> dict[str, Any]:
        hermes_profile = hermes_profile or select_hermes_profile_setting(decision_type)
        profiles = self._direct_model_profiles(hermes_profile.llm_fallback_order if hermes_profile else None)
        if not profiles:
            raise RuntimeError("real provider not configured")
        attempts: list[str] = []
        errors: dict[str, str] = {}
        last_error: Exception | None = None
        retry_count = max(0, hermes_profile.max_retries if hermes_profile else 0)
        for profile in profiles:
            ...
            for attempt_index in range(retry_count + 1):
                ...
                try:
                    payload = self._call_direct_model_profile(decision_type, context, profile, hermes_profile=hermes_profile)
                    payload.setdefault("direct_model_profile", {})
                    payload["direct_model_profile"].update({
                        "name": name,
                    ...
                    last_error = exc
                    continue
        raise RuntimeError(f"all direct_model profiles failed: {errors}") from last_error
