...
from factor_lab.hermes_research_schemas import DIAGNOSTICIAN_ENHANCEMENT_SCHEMA_VERSION
...
def _build_question_cards_v2(question_cards: list[dict[str, Any]], rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
            ...
            source_cards.append(
                {
                    ...
                    "question_type": "diagnostician_bootstrap",
    ...
    for card in source_cards:
        ...
        out.append(
            {
                **card,
                "schema_version": DIAGNOSTICIAN_ENHANCEMENT_SCHEMA_VERSION,
...
def build_diagnostician_enhancement(snapshot: dict[str, Any]) -> dict[str, Any]:
...
def write_diagnostician_enhancement(snapshot: dict[str, Any], artifacts_dir: str | Path) -> dict[str, Any]:
    artifacts_dir = Path(artifacts_dir)
    artifacts_dir.mkdir(parents=True, exist_ok=True)
    payload = build_diagnostician_enhancement(snapshot)
    (artifacts_dir / "diagnostician_enhancement.json").write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")
...
def load_diagnostician_enhancement(artifacts_dir: str | Path) -> dict[str, Any]:
    artifacts_dir = Path(artifacts_dir)
    return _load_json(
        artifacts_dir / "diagnostician_enhancement.json",
        {
