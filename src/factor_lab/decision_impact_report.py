...
def build_decision_impact_report(
    *,
    planner_brief_path: str | Path = ARTIFACTS / "researcher_profile_brief.json",
    failure_brief_path: str | Path = ARTIFACTS / "diagnostician_brief.json",
    ...
    current = _read_json(hermes_decision_artifacts_path, {})

    heuristic_router = HermesDecisionRouter(provider="heuristic")
    ...
    heuristic_failure = heuristic_router.generate("diagnostician", failure_context) if failure_context else {}

    planner_current = current.get("planner") or {}
    failure_current = current.get("diagnostician") or {}
