...
3. Runtime uses role-specific fallback first:
   - `HermesDecisionRouter._call_direct_model()` calls `_direct_model_profiles(hermes_profile.llm_fallback_order)`.
   - Therefore if `FACTOR_LAB_HERMES_PROFILE_SETTINGS_JSON` has stale role fallback names, the new global LLM fallback order from `/settings` is not necessarily used.

4. Similar stale cases found:
   - LLM profile rename/delete can leave Hermes profiles pointing to missing profile names.
   - Disabling a model profile can leave Hermes profiles pointing to a disabled profile name.
...
1. LLM profiles remain connection/provider settings; Hermes profiles remain responsibility settings.
2. Global LLM fallback order is the default provider order.
3. Hermes profiles may still have role-specific order, but every name in `role.llm_fallback_order` must be an available enabled LLM profile name after LLM settings save.
...
**Objective:** Prove that changing the global model fallback order through `/settings` updates existing Hermes profiles that were using the old global order.
...
1. `test_save_llm_settings_syncs_hermes_profile_fallback_when_roles_used_old_global_order`
   - Env starts with profiles `primary,backup` and Hermes profiles whose `llm_fallback_order` is `primary,backup`.
   - Save LLM settings with numeric order making fallback `backup,primary`.
   - Assert `FACTOR_LAB_HERMES_PROFILE_SETTINGS_JSON` in env/file now has role fallback `backup,primary`.

2. `test_save_llm_settings_filters_stale_hermes_profile_profile_names_but_preserves_custom_order`
   - Env starts with profiles `primary,backup,third` and Hermes profile fallback `third,backup`.
...
```bash
python -m pytest tests/test_webui_llm_settings.py::test_save_llm_settings_syncs_hermes_profile_fallback_when_roles_used_old_global_order tests/test_webui_llm_settings.py::test_save_llm_settings_filters_stale_hermes_profile_profile_names_but_preserves_custom_order -q
```

**Expected RED:** fail because `save_llm_settings()` currently does not update `FACTOR_LAB_HERMES_PROFILE_SETTINGS_JSON`.
...
```python
def _sync_hermes_profile_settings_with_llm_profiles(existing_values, profiles, old_fallback_order, new_fallback_order) -> dict[str, str]:
    ...
    return {
        "FACTOR_LAB_HERMES_PROFILE_SETTINGS_JSON": hermes_profile_settings_to_json(updated_roles),
        "FACTOR_LAB_HERMES_PROFILE_ORDER": existing order or joined role names,
    }
```
...
- After profiles/fallback are derived, call `_sync_hermes_profile_settings_with_llm_profiles(...)`.
- Merge returned agent env keys into `requested`.
- Include `HERMES_PROFILE_SETTING_ENV_KEYS` in `managed_keys` only when sync returns values, so existing role JSON lines can be updated in place and missing lines appended if roles already exist in env/process.
...
- Env has LLM profiles `primary` only and Hermes profile fallback `missing,primary`.
- GET `/agents`.
...
```bash
python -m pytest tests/test_webui_llm_settings.py tests/test_webui_agent_settings.py tests/test_hermes_decision_router.py -q
