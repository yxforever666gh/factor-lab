...
from factor_lab.hermes_profile_settings import (
    HermesProfileSetting,
    hermes_profile_settings_to_json,
    load_hermes_profile_settings,
    select_hermes_profile_setting,
)


def test_load_hermes_profile_settings_uses_defaults_when_env_missing(monkeypatch):
    ...
    roles = load_hermes_profile_settings()

    assert [r.name for r in roles] == ["planner", "diagnostician", "reviewer", "data_steward"]
...
def test_select_hermes_profile_setting_by_decision_type(monkeypatch):
    monkeypatch.setenv(
    ...
    assert select_hermes_profile_setting("planner").name == "planner"
    assert select_hermes_profile_setting("diagnostician").name == "diagnostician"


def test_disabled_hermes_profile_is_not_selected(monkeypatch):
    monkeypatch.setenv(
        "FACTOR_LAB_HERMES_PROFILE_SETTINGS_JSON",
        json.dumps(
            [
    ...
    assert select_hermes_profile_setting("planner") is None


def test_hermes_profile_settings_round_trip_to_json():
    roles = [
        HermesProfileSetting(
            name="planner",
    ...
    payload = json.loads(hermes_profile_settings_to_json(roles))
...
def test_invalid_roles_json_falls_back_to_defaults(monkeypatch):
    monkeypatch.setenv("FACTOR_LAB_HERMES_PROFILE_SETTINGS_JSON", "not json")
