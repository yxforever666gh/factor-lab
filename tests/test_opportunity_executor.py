...
def test_enqueue_opportunities_marks_non_workflow_tasks_as_governed(monkeypatch, tmp_path):
    opportunities_path = tmp_path / "research_opportunities.json"
    output_path = tmp_path / "opportunity_execution_plan.json"
    db_path = tmp_path / "factor_lab.db"
    opportunities_path.write_text(
        json.dumps({"opportunities": [{"opportunity_id": "opp-diag", "opportunity_type": "diagnose", "priority": 0.9}]}),
        encoding="utf-8",
    )
    ...
    monkeypatch.setattr("factor_lab.opportunity_executor.recently_finished_same_fingerprint", lambda *args, **kwargs: False)
    monkeypatch.setattr("factor_lab.opportunity_executor._queue_counts", lambda store: {"validation": 0, "exploration": 0})
    monkeypatch.setattr("factor_lab.opportunity_executor._queue_capacity", lambda: {"validation": 1, "exploration": 1})
    ...
    monkeypatch.setattr(
        ...
        lambda opportunity: {
            "task_type": "diagnostic",
            "priority": 10,
            "fingerprint": "diag::opp-diag",
            "worker_note": "validation｜test",
            "payload": {"opportunity_id": opportunity["opportunity_id"]},
        },
    )

    payload = enqueue_opportunities(opportunities_path, output_path, db_path=db_path, limit=1, queue_aware=True)

    task = ExperimentStore(db_path).get_research_task(payload["injected"][0]["task_id"])
    assert task["payload"]["governance"]["decision"] == "allow"
    assert task["payload"]["governance"]["source"] == "opportunity_executor"
    assert payload["injected"][0]["governance_decision"] == "allow"
