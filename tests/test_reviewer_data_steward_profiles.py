...
def test_data_steward_response_flags_missing_tushare_token():
    context = {
        "context_id": "ctx-data",
        "inputs": {
    ...
    payload = build_data_steward_response(context, source_label="heuristic")

    assert payload["schema_version"] == "factor_lab.data_steward_agent_response.v1"
...
def test_validate_reviewer_and_data_steward_payloads():
    reviewer_payload = build_reviewer_response({"context_id": "x", "inputs": {}})
    data_payload = build_data_steward_response({"context_id": "y", "inputs": {"latest_run": {"dataset_rows": 1}}})

    assert validate_decision_payload("reviewer", reviewer_payload) == []
    assert validate_decision_payload("data_steward", data_payload) == []
