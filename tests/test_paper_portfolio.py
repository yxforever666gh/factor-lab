...
def _ten_stock_frame() -> pd.DataFrame:
    return pd.DataFrame(
        [
            {"date": "2026-03-20", "ticker": f"T{i:03d}", "forward_return_5d": 0.01 * i, "momentum_20": float(i)}
            for i in range(10)
        ]
    )
...
def test_build_paper_portfolio_serializes_path_source_metadata_for_empty_candidate_pool(tmp_path):
    dataset_path = tmp_path / "dataset.csv"
    output_dir = tmp_path / "paper_portfolio"
    pd.DataFrame(
        [
    ...
    payload = build_paper_portfolio(
        ...
        output_dir=output_dir,
        strategy_name="paper_candidates_only",
        source_metadata={"source": "approved_universe", "dataset_path": dataset_path},
    )

    written = json.loads((output_dir / "current_portfolio.json").read_text(encoding="utf-8"))
    assert written["input_source"]["dataset_path"] == str(dataset_path)
    assert payload["input_source"]["dataset_path"] == str(dataset_path)


def test_build_paper_portfolio_honors_target_position_count(tmp_path):
    dataset_path = tmp_path / "dataset.csv"
    output_dir = tmp_path / "paper_portfolio"
    _ten_stock_frame().to_csv(dataset_path, index=False)

    payload = build_paper_portfolio(
        dataset_path=dataset_path,
        factor_definitions=[{"name": "momentum_20", "expression": "momentum_20"}],
        output_dir=output_dir,
        target_position_count=5,
    )

    assert payload["position_count"] == 5
    assert [row["ticker"] for row in payload["positions"]] == ["T009", "T008", "T007", "T006", "T005"]
    assert payload["construction"]["mode"] == "target_position_count"
    assert payload["construction"]["target_position_count"] == 5
    assert payload["construction"]["available_universe_count"] == 10


def test_build_paper_portfolio_caps_target_position_count_at_available_universe(tmp_path):
    dataset_path = tmp_path / "dataset.csv"
    output_dir = tmp_path / "paper_portfolio"
    _ten_stock_frame().to_csv(dataset_path, index=False)
    ...
    payload = build_paper_portfolio(
        ...
        factor_definitions=[{"name": "momentum_20", "expression": "momentum_20"}],
        output_dir=output_dir,
        target_position_count=100,
    )

    assert payload["position_count"] == 10
    assert payload["construction"]["selected_count"] == 10


def test_build_paper_portfolio_reports_single_name_weight_cap_warnings(tmp_path):
    dataset_path = tmp_path / "dataset.csv"
    output_dir = tmp_path / "paper_portfolio"
    _ten_stock_frame().to_csv(dataset_path, index=False)
    ...
    payload = build_paper_portfolio(
        ...
        output_dir=output_dir,
        target_position_count=5,
        single_name_weight_cap=0.1,
    )

    assert payload["constraints"]["single_name_weight_cap"] == 0.1
    assert payload["constraints"]["max_position_weight"] == 0.2
    assert "single_name_weight_cap_breached" in payload["constraint_warnings"]
