...
def test_load_agent_settings_renders_default_roles(tmp_path: Path, monkeypatch):
    ...
    assert [role["name"] for role in settings["roles"]] == ["planner", "diagnostician", "reviewer", "data_steward"]
...
def test_save_agent_settings_writes_roles_json_and_runtime_env(tmp_path: Path, monkeypatch):
    ...
    assert webui_app.os.environ["FACTOR_LAB_HERMES_PROFILE_SETTINGS_JSON"]
...
def test_llm_page_shows_hermes_profile_settings(tmp_path: Path, monkeypatch):
    env_file = tmp_path / ".env"
...
def test_agents_page_warns_about_stale_fallback_profile_names(tmp_path: Path, monkeypatch):
    ...
    env_file.write_text(
        ...
        "FACTOR_LAB_HERMES_PROFILE_SETTINGS_JSON=" + webui_app.hermes_profile_settings_to_json(roles) + "\n"
