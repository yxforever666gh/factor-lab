...
def test_execute_task_uses_subprocess_for_workflow(monkeypatch):
    ...
    def fake_popen(command, stdout=None, stderr=None, text=None, **kwargs):
...
def test_worker_runtime_limits_respect_task_type_overrides(monkeypatch):
    monkeypatch.setenv("RESEARCH_TASK_WORKER_TIMEOUT_SECONDS", "180")
    monkeypatch.setenv("RESEARCH_TASK_WORKER_TIMEOUT_SECONDS_WORKFLOW", "7")
    monkeypatch.setenv("RESEARCH_TASK_WORKER_RSS_LIMIT_MB", "123")
    monkeypatch.setenv("RESEARCH_TASK_WORKER_POLL_SECONDS", "0.25")
    monkeypatch.setenv("RESEARCH_TASK_WORKER_KILL_GRACE_SECONDS", "3")

    limits = research_queue._worker_runtime_limits("workflow")

    assert limits["timeout_seconds"] == 7
    assert limits["rss_limit_mb"] == 123
    assert limits["poll_interval"] == 0.25
    assert limits["kill_grace_seconds"] == 3.0


def test_execute_task_timeout_terminates_worker_process_group(monkeypatch):
    calls = []
    ...
    proc = DummyProc()
    monkeypatch.setenv("RESEARCH_TASK_WORKER_TIMEOUT_SECONDS", "1")
    monkeypatch.setenv("RESEARCH_TASK_WORKER_TIMEOUT_SECONDS_WORKFLOW", "1")
    monkeypatch.setenv("RESEARCH_TASK_WORKER_RSS_LIMIT_MB", "0")
    monkeypatch.setenv("RESEARCH_TASK_WORKER_KILL_GRACE_SECONDS", "0")
    monkeypatch.setenv("RESEARCH_TASK_WORKER_POLL_SECONDS", "0.1")
    monkeypatch.setattr(research_queue.subprocess, "Popen", lambda *args, **kwargs: proc)

    now = {"value": 0.0}
    monkeypatch.setattr(research_queue.time, "time", lambda: now.__setitem__("value", now["value"] + 2.0) or now["value"])
    monkeypatch.setattr(research_queue.time, "sleep", lambda seconds: None)
    ...
    monkeypatch.setattr(research_queue.os, "killpg", fake_killpg)

    try:
        research_queue.execute_task({"task_type": "workflow", "payload": {"config_path": "configs/x.json", "output_dir": "artifacts/x"}})
        assert False, "expected RuntimeError"
    except RuntimeError as exc:
        assert "research task worker timeout after 1s" in str(exc)

    assert (54321, signal.SIGTERM) in calls
    assert (54321, signal.SIGKILL) in calls
