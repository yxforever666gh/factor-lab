...
from factor_lab.legacy_runtime_review_hooks import run_data_steward_review, run_reviewer_review


def test_run_data_steward_review_writes_artifact(tmp_path):
    output = tmp_path / "data_steward_review.json"
    payload = run_data_steward_review(
        context={
            "context_id": "ctx-data",
            "inputs": {
    ...
    assert written["decision_metadata"]["hermes_profile"] == "data_steward"
...
def test_run_reviewer_review_writes_artifact(tmp_path):
    ...
    assert output.exists()
    written = json.loads(output.read_text(encoding="utf-8"))
    assert written["decision_metadata"]["hermes_profile"] == "reviewer"
