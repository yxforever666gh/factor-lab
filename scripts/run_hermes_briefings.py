...
def _planner_brief_path() -> Path:
    return _artifacts_path() / "researcher_profile_brief.json"


def _failure_brief_path() -> Path:
    return _artifacts_path() / "diagnostician_brief.json"
...
def _planner_response_path() -> Path:
    return _artifacts_path() / "researcher_profile_response.json"


def _failure_response_path() -> Path:
    return _artifacts_path() / "diagnostician_response.json"


def _hermes_decision_artifacts_path() -> Path:
    return _artifacts_path() / "hermes_decision_artifacts.json"
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--provider",
        default=(
            os.environ.get("FACTOR_LAB_HERMES_BRIEFINGS_PROVIDER")
...
def main() -> int:
    ...
    hermes_decision_artifacts_path = _hermes_decision_artifacts_path()

    planner_brief = _read_json(planner_brief_path)
    failure_brief = _read_json(failure_brief_path)
    router = HermesDecisionRouter(provider=args.provider)
    ...
    if failure_brief:
        ...
        failure_response = router.generate("diagnostician", failure_context)
        _write_json(failure_response_path, failure_response)

    _write_json(
        hermes_decision_artifacts_path,
        {
            "loaded_at_utc": _iso_now(),
            "planner": planner_response,
            "planner_errors": list((planner_response.get("decision_metadata") or {}).get("validation_errors") or []),
            "diagnostician": failure_response,
            "diagnostician_errors": list((failure_response.get("decision_metadata") or {}).get("validation_errors") or []),
        },
    )
